    ...
    div(
      ...
      div(
        fluidRow(
          column(
            width = 12,
            h2("CytoSEE:",br(),span("automatic computation and evaluation of cytometry data",style="font-family:'Georgia'; font-size:20px;"),a(href="http://bis.zju.edu.cn/cytosee/help.html","View tutorial",target="_blank",style="font-family:'Georgia'; font-size:20px;"),
               br(),
               style =
                 "font-family: 'Bell MT';
               font-size:50px;
      ...
      div(
        ...
        h4("Contact: yczhou@zju.edu.cn; mchen@zju.edu.cn",br(),sprintf("VERSION: %s","1.2.1"),
           style =
             "font-family: 'Bell MT';
           font-size:19px;
           color: #000; text-align: center;
                ...
                column(
                  ...
                  box(
                    ...
                    radioButtons(
                      ...
                      choices = c("t_SNE","FIt-SNE","PCA"),selected = "PCA",
                      inline = TRUE),
                    br(),
                    radioButtons(
...
server = function(input,output,session){
  ...
  observeEvent(input$Rdata,{
    ...
    load(inFile$datapath)
      ...
      cyto1@dmt <- cyto@dmt
      cyto1@ID2CL <- cyto@ID2CL
      cyto1@preprocess <- cyto@preprocess
      cyto1@version <- cyto@version
      cyto1@channel.use <- cyto@channel.use
      cyto1@CL_label <- cyto@CL_label
      cyto1@event.use <- cyto@event.use
      cyto1@label <- cyto@label
      cyto1@clust <- cyto@clust
      cyto1@mst <- cyto@mst
      cyto1@transform_method <- cyto@transform_method
      cyto1@ClusterID <- cyto@ClusterID
  ...
  observeEvent(input$label_button,{
    ...
    if(is.null(select_marker)){
      shinyalert("Input fileds shouldn't be empty!",
                 type = "error",
                 confirmButtonCol="#3c8dbc")
      return()
    }
    ...
    CL_label=list()
    a=cytosee_LocCL("CD4+CD8+",MaxHitsPht = 10)
    tryCatch({
      withProgress(
